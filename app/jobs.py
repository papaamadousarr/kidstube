...
def start_job(idea_id: int, series_key: str, app) -> None:
    with _lock:
    ...
    threading.Thread(target=_run, args=(idea_id, series_key, app), daemon=True).start()
...
def _run(idea_id: int, series_key: str, app) -> None:
    ...
    with build_slot():
        ...
        process = subprocess.Popen(
            [sys.executable, "-u", "-m", "pipeline", "build", series_key],
            cwd=str(REPO_ROOT),
