...
def start_job(idea_id: int, higgsfield_series_key: str, aspect_ratio: str, app) -> None:
    with _lock:
    ...
    threading.Thread(
        target=_run,
        args=(idea_id, higgsfield_series_key, aspect_ratio, app),
        daemon=True,
    ).start()
...
def _run(idea_id: int, higgsfield_series_key: str, aspect_ratio: str, app) -> None:
    try:
        with build_slot():
            _set(idea_id, status="running", message="Démarrage...")
            build_higgsfield_series(
                higgsfield_series_key,
        ...
        _set(idea_id, status="done", success=True, message="Vidéo générée avec succès.")
